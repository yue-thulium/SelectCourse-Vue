<template>
    <div>
        ...
        <!-- 编辑弹出框 -->
        <el-dialog title="请确认绑定" :visible.sync="editVisible" width="30%" center>

            <span slot="footer" class="dialog-footer">
                <el-button @click="editVisible = false">取 消</el-button>
                <el-button type="primary" @click="saveEdit" :loading="loading">确 认</el-button>
            </span>
        </el-dialog>
    </div>
</template>
...
export default {
    ...
    methods: {
        ...
        // 保存编辑
        saveEdit() {
            var id = this.tableData[this.idx].c_id;
            this.loading = true;
            getRequest(`/teaCourseSelect?method=setCourseTeacher&c_id=${id}`).then(resp => {
                var data = resp.data;
                if (data.code == 200) {
                    this.getData();
